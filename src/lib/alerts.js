...
// Internal owner alert — fires at ORDER CREATION for manual rails (Zelle/
// Venmo). Added 2026-07-23: the completed-sale alert only fires from
// finalizePaidOrder, which on manual rails runs when the OPERATOR marks the
// deposit received — so with card down (no auto-finalizing rail) new sales
// arrived silently. This is the "money is on its way — watch the bank" signal.
export async function sendOrderReservedOwnerAlert(order) {
  const apiKey = process.env.SENDGRID_API_KEY;
  const recipients = (process.env.ORDER_ALERT_TO || process.env.ALERT_EMAIL || '')
    .split(',').map((s) => s.trim()).filter(Boolean);
  if (!apiKey || recipients.length === 0) {
    console.log('[alerts] Owner reserved alert skipped (not configured) — order:', order.order_number);
    return;
  }
  ...
  const rail = order.payment_method || 'unknown';
  const items = Array.isArray(order.items) ? order.items : [];
  const itemLines = items.map((i) => `• ${i.name || i.sku} x${i.quantity}`).join('\n');

  const body = [
    `New order reserved — awaiting ${rail} payment — $${total}`,
    ``,
    `Order #: ${order.order_number}`,
    `Customer: ${order.customer_name || ''} (${order.customer_email || ''})`,
    ``,
    itemLines,
    ``,
    `Watch for a ${rail} deposit of $${total}, then mark it paid in Admin → Orders.`,
  ].join('\n');

  const html = renderBrandedEmail({
    preheader: `Order ${order.order_number} awaiting ${rail} — $${total}`,
    eyebrow: 'Internal · Order reserved',
    heading: `Awaiting ${rail} — $${total}`,
    paragraphs: [
      `Order <strong style="color:#F5F3EC;">${escapeHtml(order.order_number)}</strong> from ${escapeHtml(order.customer_name || order.customer_email || 'customer')} is reserved and awaiting a <strong style="color:#F5F3EC;">${escapeHtml(rail)}</strong> deposit of <strong style="color:#F5F3EC;">$${total}</strong>.`,
      `When it lands, mark it paid in Admin → Orders to trigger fulfillment + the confirmation email.`,
    ],
    extraHtml: emailDetailTable([
      ...items.map((i) => ({
        label: `${escapeHtml(i.name || i.sku)} <span style="color:#6E6D68;">&times;${i.quantity}</span>`,
  ...
  try {
    await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        personalizations: [{ to: recipients.map((email) => ({ email })) }],
        from: { email: process.env.FROM_EMAIL || 'orders@syngyn.co', name: 'Syngyn Sales' },
        subject: `Order awaiting ${rail} — ${order.order_number} — $${total}`,
        content: [
          { type: 'text/plain', value: body },
          { type: 'text/html', value: html },
        ],
    ...
    console.error('[alerts] Owner reserved alert failed:', err.message);
  }
}
